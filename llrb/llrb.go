...
// Get Tree's Min Item
func (t *LLRB) GetMin() Item {
	var min Item
	_, min = getMin(t.root)
	return min
}

func getMin(h *Node) (*Node, Item) {
	if h == nil {
		return nil, nil
	}
	if h.Left == nil {
	...
	var min Item
	h.Left, min = getMin(h.Left)
	return h.Left, min
}

// Get Tree's Min Item
func (t *LLRB) GetMax() Item {
	var max Item
	_, max = getMax(t.root)
	return max
}

func getMax(h *Node) (*Node, Item) {
	if h == nil {
		return nil, nil
	}
	if h.Right == nil {
